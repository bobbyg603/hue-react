...
const Bulb: React.FC<BulbProps> = (props: BulbProps) => {
  ...
  const scale = props.size ?? Size.medium;
  const color = props.on ? `rgb(${props.color.r}, ${props.color.g}, ${props.color.g})` : 'gray';
  ...
  const glowStyle = {
    background: color,
    boxShadow: props.on ? `0px 0px ${15 * scale}px ${2.5 * scale}px ${color}` : 'none',
